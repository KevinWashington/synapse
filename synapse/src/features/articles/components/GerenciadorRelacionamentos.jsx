...
import { RecomendacoesIA } from "@/features/ai";
...
function GerenciadorRelacionamentos({
  ...
  const fetchArtigosDisponiveis = async () => {
    try {
      ...
      const relacionadosIds = relacionamentos.map((r) => r.id);
      const artigosFiltrados = data.articles.filter(
        (artigo) =>
          artigo.id !== artigoId && !relacionadosIds.includes(artigo.id)
      );
                  ...
                  {artigosDisponiveis.length === 0 ? (
                    ...
                    artigosDisponiveis.map((artigo) => (
                      ...
                      >
                        ...
                        <Button
                          size="sm"
                          onClick={() => adicionarRelacionamento(artigo.id)}
                          disabled={loadingAdd}
              ...
              {relacionamentos.map((artigo) => (
                ...
                >
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => removerRelacionamento(artigo.id)}
